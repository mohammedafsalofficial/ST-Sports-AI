  ...
  return (
    <div className="flex flex-col gap-4 pb-4">
      {currentChatSession.messages.map((message) => (
        <div key={message.id} className={`px-10 flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
          <div
            className={`px-4 py-3 rounded-lg bg-background text-primary break-words ${
              message.role === "user" ? "rounded-br-none max-w-[70%] ml-auto" : "rounded-bl-none max-w-[85%] mr-auto"
            }`}
          >
